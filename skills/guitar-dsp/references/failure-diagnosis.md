...
| Drive sounds fizzy or brittle | Host-rate hard clipping, too much high-frequency input, no post filtering, DC bias | Render high-frequency alias tests; read `aliasing-oversampling.md`; improve transfer with `nonlinear-waveshaping.md`; add DC blocker and recovery low-pass |
| Oversampling improves plots but hurts feel | Filter phase/latency side effects, dry/wet mismatch, level mismatch, over-filtered recovery path | Read `aliasing-oversampling.md`; level-match renders; check latency and dry/wet phase; compare host-rate and local island output |
...
| Tone controls feel like generic EQ | Passive stack interaction missing, wrong placement, insertion loss normalized away, no source/load modeling | Read `tone-stack-modeling.md`; measure response combinations; check placement before/after distortion |
| Fuzz cleans up poorly | Input impedance ignored, bias/gain interaction missing, sustain mapped as plain gain, tone stack too clean | Read `diode-and-fuzz-circuits.md`; test reduced input level and tone-bypass behavior |
| Tube stage sounds like a plain clipper | No cathode/coupling memory, no asymmetric bias, missing bandwidth limits, no sag/recovery | Read `triode-and-tube-stage-approximation.md`; render edge-of-breakup and transient recovery fixtures |
| Cabinet feels static or disconnected | Static IR only, no speaker compression/resonance, dynamic EQ missing, depth/presence placement wrong | Read `speaker-cabinet-dynamics.md`; compare low/medium/high level response and palm-mute envelopes |

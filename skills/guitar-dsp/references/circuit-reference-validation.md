...
- Keep taps non-invasive: instrumentation must not change processing state, ordering, latency or callback allocation behavior. For runtime-toggleable probes in the same deterministic build, require sample-identical final output, using direct comparison or sample-bit hashes. If instrumentation requires a separately compiled build, use a sufficiently strict numerical comparison and document its tolerance and why compiler optimization or floating-point code generation makes bit identity an invalid expectation. Probe storage must be prepared outside processing. This is a harness check, not a requirement for SPICE/DSP bit identity.
...
6. Instrument the actual production processing path using the [non-invasive probe contract](#compare-equivalent-stages), including the applicable same-build identity or separately compiled numerical check. Compare upstream-to-downstream with golden captures under matched controls, drive, load, settling and observation bandwidth; do not validate a separately rewritten formula.

...
Read for vinyl, woven grilles, restrained vintage hardware, cups and trim. These treatments are conditional on the product and age brief; simple pedal coatings can stay with [basic recipes](material-recipes.md). For sparkle, physical scale or uncertain appearance, use [material diagnostics](material-diagnostics.md).

## Embossed vinyl grain
...
Verify physical density separately from sheet orientation using the [visual patch test](material-diagnostics.md#visual-physical-scale-test) before correcting scale.

...
A meaningful rear view may need a separately authored neutral environment/reflection treatment when the front presentation rig leaves the recess unreadable. Prefer one orbit-safe rig where practical. If lighting changes by preset, either constrain orbit to that presentation region or update/blend the rig from camera orientation so a rear treatment cannot persist into an unrelated front view. Do not change material values; compare both views against their accepted Blender references and keep the transition deterministic per viewer.

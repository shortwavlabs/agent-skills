...
Derive useful presets such as Stack, Front, Head and Controls from existing hero/detail cameras. Pedals and rack units may need different names/counts. Record evaluated camera position, view target, up vector, perspective vertical FOV (or orthographic bounds), near/far planes, target aspect, orbit distance/polar/azimuth limits and allowed zoom. Resolve Blender tracking constraints/rigs to evaluated transforms; do not expect the rig itself to transfer.

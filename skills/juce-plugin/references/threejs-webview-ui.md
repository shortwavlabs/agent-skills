...
15. Add physical semantic controls: three-way switches, input jacks and derived LEDs/plug state.
...
For a three-way amp toggle, create one `AudioParameterChoice`, ID `ampMode`, labels `Channel 1`, `Off`, `Channel 2`, default index 1. Remove redundant independent power/channel parameters and relays. Keep shipped IDs/order compatible or implement an explicit state migration; a pre-release rename must document old-state behavior.
...
API guidance checked against JUCE 9.0.1 and Three.js documentation; inspect pinned project versions before copying APIs. References: [JUCE WebSliderRelay](https://docs.juce.com/master/classjuce_1_1WebSliderRelay.html), [Three.js rendering on demand](https://threejs.org/manual/en/rendering-on-demand.html), [resource disposal](https://threejs.org/manual/en/how-to-dispose-of-objects.html), [GLTFLoader](https://threejs.org/docs/#GLTFLoader). The amp-mode pattern was exercised in a real plugin; input-jack/cable guidance is a reusable pattern to validate in the target product, not a claim that every example was implemented there.
